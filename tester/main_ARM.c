...
void test_extractBit() {
  ...
  //fprintf(grader, "%ch\n", ch);
  TEST_CONDITION((ch==1), 3.0,
		  "extractBit - ARM", "extractBit((char)2, 1) should return 1");
  ch = extractBit((char)32, 2);
  TEST_CONDITION((ch==0), 3.0,
		  "extractBit - ARM", "extractBit((char)32, 2) should return 0");
...
void test_decodeChar() {
  ...
  TEST_CONDITION((ch==MAPPING[26]), 3.0,
		  "decodeChar - ARM", "decodeChar({'0', '1', '1', '0', '1', '0'}) should return 1");
  ch = decodeChar(ch_arr1);
  TEST_CONDITION((ch==MAPPING[43]), 3.0,
		  "decodeChar - ARM", "decodeChar({'1', '0', '1', '0', '1', '1'}) should return '");
...
void test_codeToBinary() {
	...
	FILE *F_out = fopen("outARM_codetobin.txt", "w");
	...
	fflush(F_out);
	fclose(F_expect);
	F_out = fopen("outARM_codetobin.txt", "r");
	while (1) {      
		*(ch+i) = fgetc(F_out);
		if( feof(F_out)|| i==599 )
		{ 
	...
	TEST_CONDITION(!strncmp(ch, expected, 504), 6.0,
                   "codeToBinary - ARM", " ");
...
void test_binaryToText() {
	...
	FILE *F_out = fopen("outputARM_bintotext.txt", "w");
	...
	fflush(F_in);
	fflush(F_out);
	F_out = fopen("outputARM_bintotext.txt", "r");
	while (1) {      
		*(ch+i) = fgetc(F_out);
		if( feof(F_out) || i==599 )
		{ 
	...
	TEST_CONDITION(!strncmp(ch, expected, 84), 6.0,
                   "binaryToText - ARM", " ");
	for (int j=0; j<84;j++)
		fprintf(grader, "%c", expected[j]);
	fprintf(grader, "\nYour Output: \n");
...
void test_decodeFile() {
	...
	char *expected=get_expected(F_expect);
	fclose(F_expect);
	decodeFile("sample_bintocode.txt", "decodeARM_intermediate.txt", "decodeARM_final.txt", 1);
	FILE *F_out = fopen("decodeARM_final.txt", "r");
	while (1) {      
		*(ch+i) = fgetc(F_out);
		if( feof(F_out) || i==599 )
		{ 
	...
	TEST_CONDITION(!strncmp(ch, expected, 84), 30.0,
                   "decodeFile - ARM", " ");
	fprintf(grader, "Expected Output: \n");
	for (int j=0; j<84;j++)
		fprintf(grader, "%c", expected[j]);
...
void do_all_tests() {
	...
	if (cur_test==MAX_TESTS+1) {
		fprintf(grader, "\nYour overall score for decode ARM section is: %.1f / 30.0\n", overall_score);
		fprintf(grader, "\n=========================================================================\n");
	}
}
...
char* name_test_case(int i){
	switch(i) {
		case 2:
			return "extractBit - ARM";
		case 3:
			return "decodeChar - ARM";
		case 4:
			return "codeToBinary - ARM";
		case 5:
			return "binaryToText - ARM";
		case 1:
			return "decodeFile - ARM";
		default:
			return "Unknown - ARM";
	}
}
...
void handle_sigabrt() {
    ...
    if (cur_test<=MAX_TESTS) { 
	    fprintf(grader, "Program had a sigabrt (glibc double free) for test case: %s\n", name_test_case(cur_test));
	    fprintf(grader, "\n=========================================================================\n");
    }
	++cur_test;
        fflush(grader);
